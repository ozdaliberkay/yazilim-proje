...
export default function ResetPasswordPage() {
  const [state, formAction] = useActionState(resetPassword, null as any);
    ...
    <div className="min-h-screen flex items-center justify-center p-8 bg-slate-50">
      <div className="card max-w-md w-full p-12 relative overflow-hidden bg-white shadow-2xl border border-slate-200" style={{ borderRadius: '2.5rem' }}>
        <header className="mb-12 text-center">
          ...
          <h1 className="text-4xl font-black text-slate-950 tracking-tighter uppercase italic">Şifre Sıfırlama</h1>
          <p className="text-slate-400 text-[10px] font-black mt-2 uppercase tracking-widest italic opacity-70">Yeni şifrenizi belirleyin</p>
        </header>
        ...
        <form action={formAction} className="space-y-6">
          ...
          <div className="space-y-3">
            <label className="text-[10px] uppercase font-black tracking-widest text-slate-400 ml-1">Yeni Şifre</label>
            <div className="relative">
          ...
          <button type="submit" className="btn-primary w-full flex items-center justify-center gap-2 py-5 mt-8 group bg-slate-950">
            <span className="text-white">Şifreyi Sıfırla</span>
            <ArrowRight className="group-hover:translate-x-1 transition text-white" size={20} />
          </button>

          <div className="text-center mt-10 border-t border-slate-100 pt-8">
            <p className="text-[10px] text-slate-400 font-black uppercase tracking-widest">
              <Link href="/auth/login" className="text-slate-950 hover:underline">Giriş Sayfasına Dön</Link>
            </p>
          </div>
        </form>
      </div>
